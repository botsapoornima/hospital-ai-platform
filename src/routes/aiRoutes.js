...
aiRoutes.post('/conversations', (req, res) => {
  const conversation = ctx.startConversation({ patientId: req.body.patientId, channel: req.body.channel || 'web_chat' });
  res.status(201).json(conversation);
});

aiRoutes.post('/conversations/:id/messages', async (req, res) => {
  try {
    const reply = await handleMessage(req.params.id, req.body.text);
    res.json(reply);
  } catch (err) {
...
aiRoutes.get('/conversations/:id', (req, res) => {
  res.json({ conversation: ctx.getConversation(req.params.id), context: ctx.getContext(req.params.id) });
});
